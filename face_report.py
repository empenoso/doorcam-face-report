...
# 🧪🔬 Экспериментальный скрипт для тестирования производительности распознавания лиц 🔬🧪
#
# Этот скрипт является частью технического эксперимента, цель которого — оценить
# производительность библиотеки dlib при обработке видео с использованием ускорения
# на GPU (NVIDIA CUDA) на обычном домашнем компьютере.
#
# ВАЖНОЕ УВЕДОМЛЕНИЕ:
# Данный проект создан исключительно в исследовательских и образовательных целях для
# технического тестирования. Он НЕ является системой видеонаблюдения, НЕ предназначен
# для практического применения в целях мониторинга людей и НЕ хранит базы данных лиц
# или иные персональные данные. Все операции по сравнению лиц происходят в оперативной
# памяти и только на время выполнения одного запуска скрипта.
#
# Ключевые особенности:
# - Анализ видеофайлов из указанной директории для обнаружения и кодирования лиц.
# - Использование двух моделей детекции: быстрой HOG (для CPU) и точной CNN (для GPU).
# - Сравнение векторов лиц (face encodings) для поиска уникальных личностей в рамках сессии.
# - Подсчет уникальных появлений личности (один раз на каждый видеофайл).
# - Генерация итогового HTML-отчета для наглядной демонстрации результатов теста.
#
# Установка зависимостей:
# Перед использованием необходимо запустить скрипт setup_env.sh, который скомпилирует
# и установит dlib с поддержкой CUDA.
#
# Автор: Михаил Шардин https://shardin.name/
# Дата создания: 19.08.2025
# Версия: 1.0 (Experimental)
#
